...
func (p *Init) delete(ctx context.Context) error {
	...
	var err error
	if p.FuseAbort {
		watchdogCtx, cancel := context.WithTimeout(ctx, 3*time.Second)
...
func (p *Init) abortFuse() {
	...
	if p.PodUID != "" {
		if err := utils.AbortPodFuseConnections(p.PodUID); err != nil {
			log.L.Warningf("Failed to abort pod FUSE connections for container %q (pod %q): %v", p.id, p.PodUID, err)
		}
		return
	}
	if p.Bundle != "" {
...
func (p *Init) stateRuntime(ctx context.Context) (string, error) {
	if p.FuseAbort {
		watchdogCtx, cancel := context.WithTimeout(ctx, 3*time.Second)
		state, err := p.initState.State(watchdogCtx)
		cancel()
		if err != nil && errors.Is(watchdogCtx.Err(), context.DeadlineExceeded) && ctx.Err() == nil {
			log.L.Warningf("Container %q state query wedged in Kernel.Pause, aborting FUSE connections", p.id)
			p.abortFuse()
			return p.initState.State(ctx)
		}
		return state, err
