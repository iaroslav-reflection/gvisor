...
func mapGCSError(err error, op, bucket, object string) error {
	...
	if code, ok := httpCodeFromError(err); ok {
		...
		case isPermissionDeniedCode(code):
			log.Warningf("gcs.%s on gs://%s/%s failed with permission denied (HTTP %d): %v", op, bucket, object, code, err)
			return unix.EACCES
		case isNotFoundCode(code):
			log.Warningf("gcs.%s on gs://%s/%s failed: bucket or object not found (HTTP 404): %v", op, bucket, object, err)
			return unix.ENOENT
		default:
			log.Warningf("gcs.%s on gs://%s/%s failed with HTTP %d: %v", op, bucket, object, code, err)
		}
	} else {
		log.Warningf("gcs.%s on gs://%s/%s failed: %v", op, bucket, object, err)
	}
	return err
}
